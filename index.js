...
const { color, bgcolor } = require('./FakhriGanz/color')
const { help } = require('./FakhriGanz/help')
const kagApi = require('@kagchi/kag-api')
const { donasi } = require('./FakhriGanz/donasi')
const { fetchJson } = require('./FakhriGanz/fetcher')
const { recognize } = require('./FakhriGanz/ocr')
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, getRandom, banner, start, info, success, close } = require('./FakhriGanz/functions')
const tiktod = require('tiktok-scraper')
const ffmpeg = require('fluent-ffmpeg')
const { removeBackgroundFromImageFile } = require('remove.bg')
const welkom = JSON.parse(fs.readFileSync('./FakhriGanz/welkom.json'))
const nsfw = JSON.parse(fs.readFileSync('./FakhriGanz/nsfw.json'))
const samih = JSON.parse(fs.readFileSync('./FakhriGanz/simi.json'))
const vcard = 'BEGIN:VCARD\n' // ANAK ANJING MAU NGAPAIN?
            + 'VERSION:3.0\n' // NGAPAIN LAGI KALO GA MAU NUMPANG NAMA DOANG XIXIXIXI
            + 'FN:CO NOEERBOT\n' // MENDING LU TOBAT SU!
            + 'ORG:Creator NOEERBOT;\n' // KASIH CREDITS GUA SU!!!
            + 'TEL;type=CELL;type=VOICE;waid=628124002230:+62 812-4002-238\n' // JANGAN KEK BABI SU
            + 'END:VCARD' // Fakhri
prefix = '!'
...
const config = {
    A187: '👾FakhriBot👾', // TOBAT SU ASU
    instagram: 'https://instagram.com/fakhri2005', // INFO JANGAN DI UBAH
    nomer: 'wa.me/628124002238', // INFO SU JNGAN DI UBAH
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code is ready, subscribe FakhriGanz`)
})
...
// client.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log(`${time}: Bot by ig:@fakhri2005`)
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["628124002238@s.whatsapp.net"] // ganti nomer lu
           ...
           case 'demote':
              ...
              case 'abangjago':
              case 'abgjago':
                    let eerrr = fs.readFileSync('./FakhriGanz/abangjago.mp3')
				...
				case 'tts':
					...
					const gtts = require('./FakhriGanz/gtts')(args[0])
